...
mousereport(XEvent *e)
{
	...
	if (e->type == MotionNotify) {
		 ...
		 * buttons are pressed. */
		for (btn = 1; btn <= 11 && !(buttons & (1 << (btn - 1)));
		     btn++) {
			;
		}
		code = 32;
...
xinit(int cols, int rows)
{
	...
	xw.bpointer = XCreatePixmapCursor(xw.dpy, blankpm, blankpm, &xmousefg,
	                                  &xmousebg, 0, 0);
	...
	if (dmode & DRAW_FG) {
		...
		if (base.mode & ATTR_UNDERLINE) {
			...
			const int widthThreshold =
				28;    // +1 width every widthThreshold px of
			               // font
			int wlw = (win.ch / widthThreshold) +
			          1;    // Wave Line Width
			int linecolor;
			if ((base.ucolor[0] >= 0) &&
			    !(base.mode & ATTR_BLINK &&
			      win.mode & MODE_BLINK) &&
			...
			// Underline Style
			if (base.ustyle != 3) {
				// XftDrawRect(xw.draw, fg, winx, winy +
				...
				XDrawArcs(xw.dpy, XftDrawDrawable(xw.draw), ugc,
				          arcs, narcs);
				...
				*/
				...
				// Calculate number of points with floating
				// precision
				float n = width;    // Width of word in pixels
				n = (n / ww) *
				    2;              // Number of slopes (/ or \)
				...
				if (npoints >= 3) {
					// We add an aditional slot in case we
					// use a bonus point
					XPoint *points = xmalloc(
						sizeof(XPoint) * (npoints + 1));

					// First point (Starts with the word
					// bounds)
					points[0] = (XPoint) {
						.x = wx + marginStart,
						.y = (isSlopeRising(wx, 0,
						                    ww)) ?
						             (wy - marginStart +
						              ww / 2.f) :
						             (wy + marginStart)
					...
					// Second point (Goes back to the
					// absolute point coordinates)
					points[1] = (XPoint) {
						.x = (ww / 2.f) - marginStart,
						.y = (isSlopeRising(wx, 1,
						                    ww)) ?
						             (ww / 2.f -
						              marginStart) :
						             (-ww / 2.f +
					...
					// Draw the lines
					XDrawLines(xw.dpy,
					           XftDrawDrawable(xw.draw),
					           ugc, points, npoints,
					           CoordModePrevious);

					// Draw a second underline with an
					// offset of 1 pixel
					if (((win.ch / (widthThreshold / 2)) %
					     2)) {
						points[0].x++;
         ...
         // Cap is half of wave width
				...
				// Set the angle of the slope to 45Â°
				ww = wh;
				ww *= 1 + capRatio;    // Add a bit of width for
				                       // the cap
...
void
xdrawcursor(int cx, int cy, Glyph g, int ox, int oy, Glyph og, Line line,
            int len)
{
	Color drawcol;
	...
	if (IS_SET(MODE_FOCUSED)) {
		...
		case 6: /* Steady bar */
			XftDrawRect(xw.draw, &drawcol, borderpx + cx * win.cw,
			            borderpx + cy * win.ch, cursorthickness,
			            win.ch);
			break;
		}
	} else {
...
xdrawline(Line line, int x1, int y1, int x2)
{
	...
	numspecs_cached =
		xmakeglyphfontspecs(xw.specbuf, &line[x1], x2 - x1, x1, y1);
...
run(void)
{
	...
	for (timeout = -1, drawing = 0, lastblink = (struct timespec) { 0 };;) {
		...
		if (pselect(MAX(xfd, ttyfd) + 1, &rfd, NULL, NULL, tv, NULL) <
		    0) {
			if (errno == EINTR) {
				continue;
			}
